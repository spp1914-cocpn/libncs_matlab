classdef NcsPktGetTimeStampTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testInvalidDataPacket(this)
            expectedErrId = 'ncs_pktGetTimeStamp:InvalidPacket';
            
            invalidPacket = this; % not a data packet
            this.verifyError(@() ncs_pktGetTimeStamp(invalidPacket), expectedErrId);
        ...
        %% test
        function test(this)
            actualTimestamp = ncs_pktGetTimeStamp(this.dataPacket);
            
            this.verifyEqual(actualTimestamp, this.timestamp);
        end
    end
end
